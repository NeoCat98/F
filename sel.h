...
Matrix createLocalK(int element,mesh &m){
    float D,Ae,k = m.getParameter(K_CTE);
    Matrix K,A,B,Bt,At;
    ...
    transpose(A,At);
    transpose(B,Bt);
    productRealMatrix((-1)*k*Ae/(2*D*D),productMatrixMatrix(Bt,productMatrixMatrix(At,productMatrixMatrix(A,B,2,2,3),2,2,3),3,2,3),K);
...
Vector createLocalb(int element,mesh &m){
    ...
    float f = m.getParameter(F_CTE),J;
